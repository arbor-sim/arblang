...
p_expr normalize(const parsed_parameter& e) {
    ...
    return make_expr<parsed_parameter>(e.identifier, val, e.loc);
}
p_expr normalize(const parsed_constant& e) {
    auto val = std::visit([&](auto&& c){return normalize(c);}, *e.value);
    return make_expr<parsed_constant>(e.identifier, val, e.loc);
}
p_expr normalize(const parsed_state& e) {
    return make_expr<parsed_state>(e);
}
p_expr normalize(const parsed_record_alias& e) {
    return make_expr<parsed_record_alias>(e);
}
p_expr normalize(const parsed_function& e) {
    auto body = std::visit([&](auto&& c){return normalize(c);}, *e.body);
    return make_expr<parsed_function>(e.name, e.args, e.ret, body, e.loc);
}
p_expr normalize(const parsed_bind& e) {
    return make_expr<parsed_bind>(e);
}
p_expr normalize(const parsed_initial& e) {
    auto val = std::visit([&](auto&& c){return normalize(c);}, *e.value);
    return make_expr<parsed_initial>(e.identifier, val, e.loc);
}
p_expr normalize(const parsed_evolve& e) {
    auto val = std::visit([&](auto&& c){return normalize(c);}, *e.value);
    return make_expr<parsed_evolve>(e.identifier, val, e.loc);
}
p_expr normalize(const parsed_effect& e) {
    auto val = std::visit([&](auto&& c){return normalize(c);}, *e.value);
    return make_expr<parsed_effect>(e.effect, e.ion, e.type, val, e.loc);
}
p_expr normalize(const parsed_export& e) {
    return make_expr<parsed_export>(e);
}
p_expr normalize(const parsed_call& e) {
    ...
    return make_expr<parsed_call>(e.function_name, args, e.loc);
}
p_expr normalize(const parsed_object& e) {
    ...
    return make_expr<parsed_object>(e.record_name, e.record_fields, record_values, e.loc);
}
p_expr normalize(const parsed_let& e) {
    auto val = std::visit([&](auto&& c){return normalize(c);}, *e.value);
    auto body = std::visit([&](auto&& c){return normalize(c);}, *e.body);
    return make_expr<parsed_let>(e.identifier, val, body, e.loc);
}
p_expr normalize(const parsed_with& e) {
    auto val = std::visit([&](auto&& c){return normalize(c);}, *e.value);
    auto body = std::visit([&](auto&& c){return normalize(c);}, *e.body);
    return make_expr<parsed_with>(val, body, e.loc);
}
p_expr normalize(const parsed_conditional& e) {
    ...
    return make_expr<parsed_conditional>(cond, val_true, val_false, e.loc);
}
p_expr normalize(const parsed_identifier& e) {
    return make_expr<parsed_identifier>(e);
}
p_expr normalize(const parsed_unary& e) {
    auto val = std::visit([&](auto&& c){return normalize(c);}, *e.value);
    return make_expr<parsed_unary>(e.op, val, e.loc);
}
p_expr normalize(const parsed_binary& e) {
    ...
    return make_expr<parsed_binary>(e.op, lhs, rhs, e.loc);
}
p_expr normalize(const parsed_float& e) {
    ...
    if (val == (int)val) {
        return make_expr<parsed_int>(val, unit.first, e.loc);
    }
    return make_expr<parsed_float>(val, unit.first, e.loc);
}
