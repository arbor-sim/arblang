...
printable_mechanism::printable_mechanism(const resolved_mechanism& m) {
    ...
    for (const auto& s: m.states) {
        ...
        if (!state_rec) {
            ...
            field_pack.state_sources.insert(state_name);
        }
        else {
            for (const auto&[field_name, field_type]: state_rec->fields) {
                ...
                field_pack.state_sources.insert(state_field_name);
            }
        }
    }
    for (const auto& c: m.bindings) {
        auto bind = std::get<resolved_bind>(*c);
        pointer_map.insert({bind.name, prefix(bind.name)});
        field_pack.bind_sources.insert({bind.name, {bind.bind, bind.ion}});
    }
    for (const auto& c: m.parameters) {
        ...
        field_pack.param_sources.insert({param.name, {val, to_string(type)}});
    }
    for (const auto& c: m.effects) {
        ...
        pointer_map.insert({"i", prefix("i")});
        field_pack.effect_sources.insert({"i", {i_effect, eff.ion}});

        pointer_map.insert({"g", prefix("g")});
        field_pack.effect_sources.insert({"g", {g_effect, eff.ion}});
    }
        ...
        if (auto vtype = std::get_if<resolved_record>(type.get())) {
            ...
            for (const auto& v: results) {
                ...
                if (is_state) {
                    ...
                    auto pointer_name = pointer_map.at(field_mangled_name);
                    ...
                    auto pointer_name = pointer_map.at(field_name);
                    map.insert({pointer_name, field_value->name});
                }
            }
        }
            ...
            auto pointer_name  = pointer_map.at(vname);
...
void printable_mechanism::fill_read_maps() {
    for (const auto& c: procedure_pack.assigned_parameters) {
        ...
        for (const auto& a: read_args) {
            ...
            if (field_pack.param_sources.count(a)) {
    ...
    for (const auto& c: procedure_pack.initializations) {
        ...
        for (const auto& a: read_args) {
            ...
            else if ((field_pack.bind_sources.count(a))) {
    ...
    for (const auto& c: procedure_pack.evolutions) {
        ...
        for (const auto& a: read_args) {
            ...
            else if ((field_pack.state_sources.count(a))) {
